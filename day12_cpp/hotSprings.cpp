...
Terms:
n = length of the pattern (including the additional '.' at the end)
m = number of damaged positions

The algorithm explores all possible combinations of placing dominoes while respecting the damaged positions.
It uses memorization to avoid redundant computations of subproblems.
...
struct memo {
    memo(std::string_view pattern, std::span<const int> damaged)
        : data(pattern.length() + 1, std::vector<int>(damaged.size() + 1, -1)) {}
    std::vector<std::vector<int>> data;
 
    struct node {
        int& value;
        explicit operator bool() const { return value != -1; }
        int memoize(int v) { return value = v; }
    };
...
int count(std::string_view pattern, std::span<const int> damaged, memo& m) {
...
int count(const input& in) {
    std::string aug_pattern = in.pattern + '.';
    memo m(aug_pattern, in.damaged);
    return count(aug_pattern, in.damaged, m);
}
 
int solve(std::span<const input> inputs) {
    int found = 0;
    for (auto& in : inputs) {
        found += count(in);
    }
